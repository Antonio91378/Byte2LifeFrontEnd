  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center border-b-2 border-brand-orange pb-4 gap-4">
        <h1 className="text-3xl font-bold text-brand-purple">Estoque de Impressões</h1>
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          ...
          >
            ...
            <option value="Available" className="text-gray-900">Disponíveis</option>
            <option value="Sold" className="text-gray-900">Vendidos</option>
          </select>

          <Link href="/stock/new" className="bg-brand-purple hover:bg-purple-800 text-white px-4 py-2 rounded-lg transition-colors flex items-center gap-2 shadow-md justify-center">
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
          ...
          <p className="mt-1 text-sm text-gray-500">Tente ajustar os filtros ou adicione um novo item.</p>
        </div>
      ) : (
...
function StockCard({ item, getFilamentName, onDelete, onSelect }: { 
  item: StockItem, 
    ...
    >
      <div className="h-48 bg-gray-100 relative overflow-hidden group">
        {item.photos && item.photos.length > 0 ? (
          <>
            <img 
              src={`http://localhost:5000${item.photos[currentPhotoIndex]}`} 
        ...
        <div className="absolute top-2 right-2">
          <span className={`px-2 py-1 text-xs font-bold rounded-full ${item.status === 'Available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="text-lg font-bold text-gray-800 mb-1 truncate" title={item.description}>{item.description}</h3>
        <p className="text-sm text-gray-500 mb-4 truncate">{getFilamentName(item.filamentId)}</p>
        
        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mb-4">
          <div>
            <span className="block text-xs text-gray-400 uppercase">Custo Prod.</span>
            <span className="font-medium text-gray-700">R$ {(item.productionCost || 0).toFixed(2)}</span>
          </div>
          <div>
            <span className="block text-xs text-gray-400 uppercase">Valor Sugerido</span>
            <span className="font-bold text-green-600">R$ {item.cost.toFixed(2)}</span>
          </div>
        </div>

        <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center gap-2" onClick={e => e.stopPropagation()}>
          <Link 
...
function SideDrawer({ item, isOpen, onClose, getFilamentName }: { 
  item: StockItem | null, 
  isOpen: boolean, 
  onClose: () => void,
  getFilamentName: (id: string) => string
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 h-full w-full md:w-[480px] bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto`}>
        <div className="p-6 space-y-6">
          ...
          <div className="aspect-video bg-gray-100 rounded-xl overflow-hidden">
            {item.photos && item.photos.length > 0 ? (
              <img src={`http://localhost:5000${item.photos[0]}`} alt={item.description} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-gray-400">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Filamento</span>
              <span className="text-gray-900 font-medium">{getFilamentName(item.filamentId)}</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Peso</span>
              <span className="text-gray-900 font-medium">{item.weightGrams}g</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Tempo de Impressão</span>
              <span className="text-gray-900 font-medium">{item.printTime}</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Qualidade</span>
              <span className="text-gray-900 font-medium">{item.printQuality || 'Normal'}</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Nozzle</span>
              <span className="text-gray-900 font-medium">{item.nozzleDiameter || '-'}</span>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl">
              <span className="block text-xs text-gray-500 uppercase font-bold mb-1">Camada</span>
              <span className="text-gray-900 font-medium">{item.layerHeight || '-'}</span>
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl border border-gray-200">
                ...
                <span className="text-xl font-bold text-gray-700">R$ {(item.productionCost || 0).toFixed(2)}</span>
              </div>
              <div className="p-4 rounded-xl bg-green-50 border border-green-100">
                <span className="block text-xs text-green-600 uppercase font-bold mb-1">Valor de Venda</span>
                <span className="text-xl font-bold text-green-700">R$ {item.cost.toFixed(2)}</span>
              </div>
            </div>
            <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-100 flex justify-between items-center">
              <span className="text-sm font-medium text-blue-800">Lucro Estimado</span>
              <span className="text-lg font-bold text-blue-900">R$ {(item.cost - (item.productionCost || 0)).toFixed(2)}</span>
            </div>
          </div>

          {/* Extras */}
          <div className="border-t border-gray-100 pt-6">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Acabamentos</h3>
            <div className="flex gap-3">
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${item.hasCustomArt ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-400'}`}>
                Arte Personalizada
              </span>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${item.hasPainting ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-400'}`}>
                Pintura
              </span>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${item.hasVarnish ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-400'}`}>
                Verniz
              </span>
            </div>
          </div>
