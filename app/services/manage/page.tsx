  ...
  const handleProviderSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (editingProviderId) {
        ...
        setProviders(prev =>
          prev.map(provider =>
            provider.id === editingProviderId ? { ...provider, name, email, categories } : provider
          )
        );
                        ...
                        {record.source === 'sale' && record.saleId ? (
                          ...
                          <div className="flex items-center justify-end gap-3">
                            <button
                              type="button"
                              onClick={() => {
                                if (record.category === 'design' && record.taskId) {
                                  const task = designTasks.find(item => item.id === record.taskId);
                                  if (task) {
                                    handleTaskEdit(task);
                                  }
                                }
                                if (record.category === 'painting' && record.taskId) {
                                  const task = paintingTasks.find(item => item.id === record.taskId);
                                  if (task) {
                                    handlePaintingEdit(task);
                                  }
                                }
