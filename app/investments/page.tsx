...
const InvestmentsChart = dynamic(
  () => import("@/components/InvestmentsChart"),
  { ssr: false },
);
...
function getServiceDateStr(task: any) {
  return (
    task.startAt || task.StartAt || task.createdAt || task.CreatedAt || null
  );
}
...
function getMonthRange(month: string, monthCount: number) {
  const [year, monthIndex] = month.split("-").map(Number);
  const range: string[] = [];

  for (let offset = monthCount - 1; offset >= 0; offset -= 1) {
    const currentDate = new Date(year, monthIndex - 1 - offset, 1);
    range.push(
...
function filterChartEntriesByLabels(
  entries: { label: string; value: number; description: string }[],
  labels: string[],
) {
...
function aggregateServicesByMonth(tasks: any[]) {
  const map = new Map<string, { value: number; description: string }>();
  tasks.forEach((task) => {
    const dateStr = getServiceDateStr(task);
    if (!dateStr) return;
...
function mergeChartEntries(
  ...
  entries.forEach((entry) => {
    const prev = map.get(entry.label) || {
      value: 0,
      description: `Lucro em ${entry.label}`,
    };
    map.set(entry.label, {
...
export default function InvestmentsPage() {
  ...
  const [selectedRangeMonths, setSelectedRangeMonths] = useState(1);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `http://localhost:5000/api/investments/${editingId}`,
          formData,
        );
      } else {
  ...
  const allMonths = useMemo(() => {
    ...
    return Array.from(monthsSet).sort((firstMonth, secondMonth) =>
  ...
  const selectedMonthRange = useMemo(() => {
    ...
    return getMonthRange(selectedMonth, selectedRangeMonths);
  }, [selectedMonth, selectedRangeMonths]);

  const rangeLabel = useMemo(() => {
    if (selectedMonthRange.length === 0) return "";
    if (selectedMonthRange.length === 1) return selectedMonthRange[0];
    return `${selectedMonthRange[0]} a ${selectedMonthRange.at(-1)}`;
  }, [selectedMonthRange]);
  ...
  const servicesByDay = useMemo(() => {
    if (!selectedMonth) return [];
    return aggregateServicesByDay(
      [...designTasks, ...paintingTasks],
      selectedMonth,
    );
  ...
  const profitByMonthRange = useMemo(() => {
    if (selectedMonthRange.length === 0) return [];
    ...
    const servicesByMonth = filterChartEntriesByLabels(
      aggregateServicesByMonth([...designTasks, ...paintingTasks]),
      selectedMonthRange,
    );

    return mergeChartEntries([...salesByMonth, ...servicesByMonth]);
  }, [designTasks, paintingTasks, sales, selectedMonthRange]);

  const investmentsByMonthRange = useMemo(() => {
    if (selectedMonthRange.length === 0) return [];
    return filterChartEntriesByLabels(
      aggregateInvestmentsByMonth(investments),
      selectedMonthRange,
    );
  }, [investments, selectedMonthRange]);
  ...
  const chartLabels = useMemo(() => {
    ...
    if (selectedRangeMonths === 1) {
      ...
      const labels: string[] = [];
      const currentDate = new Date(year, monthIndex - 1, 1);

      while (currentDate.getMonth() === monthIndex - 1) {
        labels.push(
  ...
  }, [selectedMonth, selectedMonthRange, selectedRangeMonths]);

  const chartSales =
    selectedRangeMonths === 1 ? profitByDay : profitByMonthRange;
  const chartInvestments =
    selectedRangeMonths === 1 ? investmentsByDay : investmentsByMonthRange;
  const chartSummaryLabel =
    selectedRangeMonths === 1
      ? "Resumo do mês"
      : `Resumo de ${selectedRangeMonths} meses`;
  const chartTitle = rangeLabel
    ? `Lucro x Investimentos - ${rangeLabel}`
    : "Lucro x Investimentos";
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-0 pb-12">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        <div className="flex items-center gap-2">
          <label
            htmlFor="investment-month-filter"
            className="font-medium text-gray-700"
          >
            Mês final:
          </label>
          <select
            id="investment-month-filter"
            ...
            value={
              selectedMonth && allMonths.includes(selectedMonth)
                ? selectedMonth
                : allMonths[0] || ""
            }
        ...
        <div className="flex items-center gap-2">
          <label
            htmlFor="investment-range-filter"
            className="font-medium text-gray-700"
          >
            Período:
          </label>
          <select
            id="investment-range-filter"
            className="p-2 border border-gray-300 rounded-lg text-gray-800 bg-white focus:ring-2 focus:ring-brand-purple focus:border-transparent"
            value={selectedRangeMonths}
            onChange={(e) => setSelectedRangeMonths(Number(e.target.value))}
          >
        ...
        {rangeLabel && (
          <span className="text-sm font-medium text-gray-500 sm:ml-2">
            Intervalo exibido: {rangeLabel}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-green-100 relative overflow-hidden">
          <div className="absolute right-0 top-0 p-4 opacity-10">
            ...
            >
              <path
                fillRule="evenodd"
                d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 8.586 15.586 4H12z"
                clipRule="evenodd"
              ></path>
            </svg>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-red-100 relative overflow-hidden">
          <div className="absolute right-0 top-0 p-4 opacity-10">
            ...
            >
              <path
                fillRule="evenodd"
                d="M12 13a1 1 0 100 2h5a1 1 0 001-1V9a1 1 0 10-2 0v2.586l-4.293-4.293a1 1 0 00-1.414 0L8 9.586 3.707 5.293a1 1 0 00-1.414 1.414l5 5a1 1 0 001.414 0L11 9.414 15.586 14H12z"
                clipRule="evenodd"
              ></path>
            </svg>
          </div>
        ...
        {/* Balance / ROI */}
        <div
          className={`bg-white p-6 rounded-xl shadow-sm border relative overflow-hidden ${balance >= 0 ? "border-brand-purple" : "border-orange-200"}`}
        >
          <h3 className="text-sm font-bold text-gray-500 uppercase mb-1">
            Balanço (ROI)
          </h3>
          <p
            className={`text-3xl font-bold ${balance >= 0 ? "text-brand-purple" : "text-orange-600"}`}
          >
          ...
          <p className="text-xs text-gray-500 mt-1 text-right">
            {progress.toFixed(1)}% do investimento recuperado
          </p>
        </div>
      </div>
