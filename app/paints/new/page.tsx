...
export default function NewPaintPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/paints', formData);
      await showAlert('Sucesso', 'Tinta cadastrada com sucesso!', 'success');
      router.push('/paints');
    } catch (error) {
      console.error(error);
      await showAlert('Erro', 'Erro ao cadastrar tinta', 'error');
    }
  };

  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold text-brand-purple mb-8">Nova Tinta</h1>
