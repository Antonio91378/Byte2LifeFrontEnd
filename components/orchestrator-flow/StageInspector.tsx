...
function DocsRenderer({ text, borderColor }: { text: string; borderColor: string }) {
  ...
  function renderInline(raw: string): React.ReactNode[] {
    ...
    while ((m = regex.exec(raw)) !== null) {
      ...
      parts.push(
        <span key={m.index} style={{ fontFamily: 'monospace', background: '#ffffff14', color: '#22d3ee', borderRadius: 3, padding: '0 4px', fontSize: '0.95em' }}>
          {m[1]}
        </span>
      );
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <div key={i} style={{ fontFamily: 'monospace', fontSize: 9, color: borderColor, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginTop: 14, marginBottom: 5, borderBottom: `1px solid ${borderColor}33`, paddingBottom: 3 }}>
          {line.slice(3)}
        </div>
      );
      ...
      elements.push(
        <div key={i} style={{ display: 'flex', gap: 7, marginBottom: 3, alignItems: 'flex-start' }}>
          <span style={{ color: borderColor, flexShrink: 0, marginTop: 1 }}>·</span>
          <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#ffffffaa', lineHeight: 1.6 }}>
      ...
      elements.push(
        <div key={i} style={{ fontFamily: 'monospace', fontSize: 10, color: '#ffffffaa', lineHeight: 1.65, marginBottom: 2 }}>
          {renderInline(line)}
        </div>
      );
    }
  ...
  async function handleSaveProvider() {
    ...
    try {
      const fallback = editFallbackProvider
        ? { provider: editFallbackProvider, model: editFallbackModel || undefined }
        : undefined;
      ...
      onSkillUpdated?.(skillKey, { provider: editProvider, model: editModel } as LLMSkillConfig);
      setEditingLLM(false);
    } catch {
      // show inline error
    } finally {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0 }}>
          <button
            ...
            title="Diminuir zoom"
            style={{ background: '#ffffff0d', border: '1px solid #ffffff1a', color: '#ffffff55', fontFamily: 'monospace', fontSize: 13, width: 20, height: 20, borderRadius: 4, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, lineHeight: 1 }}
          >
            −
          </button>
          <span style={{ fontFamily: 'monospace', fontSize: 8, color: '#ffffff33', minWidth: 26, textAlign: 'center' }}>
            {Math.round(zoom * 100)}%
          </span>
          <button
            type="button"
      ...
      <div style={{ zoom, padding: '14px 14px' }}>
        ...
        {stage.description && (
          <Section title="O que faz" color={colors.border}>
            <p style={{ fontFamily: 'monospace', fontSize: 10, color: '#ffffffaa', lineHeight: 1.6, margin: 0 }}>
              {descExpanded || (stage.description.length <= 120 && !stage.docs)
                ? stage.description
                : `${stage.description.slice(0, 120)}…`}
            </p>
            {descExpanded && stage.docs && (
              <div style={{ marginTop: 12, borderTop: `1px solid ${colors.border}22`, paddingTop: 4 }}>
            ...
            {(stage.description.length > 120 || stage.docs) && (
              <button
                ...
                style={{ fontFamily: 'monospace', fontSize: 8, color: `${colors.border}99`, background: 'none', border: 'none', cursor: 'pointer', padding: '6px 0 0', letterSpacing: '0.05em' }}
              >
        ...
        {/* Skill / LLM + editor */}
        {skillConfig && (() => {
          const isVision = !!(skillConfig as unknown as LLMSkillConfig).requiresVision;
          const fallback = skillConfig.fallback as { provider: string; model?: string } | undefined;

          const inputSt: React.CSSProperties = {
            fontFamily: 'monospace', fontSize: 10, background: '#120020', color: '#ffffff',
            border: '1px solid #a78bfa44', borderRadius: 4, padding: '4px 7px',
            width: '100%', boxSizing: 'border-box', outline: 'none',
          };
          const labelSt: React.CSSProperties = {
            fontFamily: 'monospace', fontSize: 8, color: '#a78bfaaa',
              ...
              <div style={{ padding: '8px 10px', border: '1px solid #a78bfa44', borderRadius: 6, background: '#a78bfa11' }}>
                {!editingLLM ? (
                  <>
                    {!!skillConfig.model && (
                      <div style={{ display: 'flex', gap: 6, marginBottom: 4, alignItems: 'center' }}>
                        <BrainCircuit size={10} style={{ color: '#a78bfa', flexShrink: 0 }} />
                        {isVision && <span style={{ fontSize: 10 }}>👁</span>}
                        <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#a78bfacc', flex: 1 }}>
                        ...
                        {baseUrl && (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditFallbackProvider(fallback?.provider ?? '');
                              setEditFallbackModel(fallback?.model ?? '');
                              setEditingLLM(true);
                            }}
                            style={{ background: 'none', border: '1px solid #a78bfa44', borderRadius: 4, color: '#a78bfa88', cursor: 'pointer', padding: '1px 4px', display: 'flex', alignItems: 'center' }}
                          >
                            <Pencil size={8} />
                          </button>
                        )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <div>
                      ...
                      >
                        ...
                        {(providers ?? []).filter(p => p.kind === 'cloud').map(p => (
                          <option key={p.id} value={p.id}>{p.label ?? p.id} (cloud)</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      <label style={labelSt}>{isVision ? 'Modelo vision 👁' : 'Modelo'}</label>
                      {baseUrl && isVision ? (
                        <ModelSelector
                          baseUrl={baseUrl}
                      ...
                      {isVision && (
                        <div style={{ fontFamily: 'monospace', fontSize: 8, color: '#ffffff33', marginTop: 3, lineHeight: 1.5 }}>
                          Apenas modelos multimodais. Para Ollama: <code style={{ color: '#a78bfa' }}>ollama pull llava:13b</code>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => { setEditFallbackProvider(e.target.value); setEditFallbackModel(''); }}
                        style={{ ...inputSt, cursor: 'pointer', marginBottom: 5 }}
                      >
                      ...
                      {editFallbackProvider && (
                        <>
                          <label style={{ ...labelSt, marginTop: 3 }}>Fallback modelo</label>
                          {baseUrl && isVision && editFallbackProvider ? (
                            ...
                            <input
                              ...
                              style={{ ...inputSt, borderColor: '#22d3ee44' }}
                            />
                          )}
                        </>
                      )}
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button type="button" onClick={handleSaveProvider} disabled={saving} style={{ fontFamily: 'monospace', fontSize: 9, background: saving ? '#a78bfa33' : '#a78bfa22', color: '#a78bfa', border: '1px solid #a78bfa55', borderRadius: 4, padding: '4px 10px', cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Save size={8} /> {saving ? 'Salvando…' : 'Salvar'}
                      </button>
                      <button type="button" onClick={() => setEditingLLM(false)} style={{ fontFamily: 'monospace', fontSize: 9, background: 'none', color: '#ffffff44', border: '1px solid #ffffff22', borderRadius: 4, padding: '4px 10px', cursor: 'pointer' }}>
                        Cancelar
                      </button>
                    </div>
                  </div>
        ...
        {skillKey === 'image_generation' && (
          <Section title="Providers de Imagem" color="#f97316">
            <div style={{ fontFamily: 'monospace', fontSize: 9, color: '#f97316aa', lineHeight: 1.7 }}>
              Clique no node <span style={{ color: '#f97316', fontWeight: 700 }}>Image Generation</span> no canvas para abrir o sub-fluxo interativo de workflows.
            </div>
          </Section>
        )}
