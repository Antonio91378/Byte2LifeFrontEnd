...
export default function Reminders() {
  ...
  const upsertLocalReminder = (items: Reminder[], payload: { title: string; isDone: boolean }, id?: string | null) => {
    if (id) {
      return items.map(item =>
        item.id === id
          ? { ...item, title: payload.title, isDone: payload.isDone, updatedAt: new Date().toISOString() }
          : item
      );
    }
    ...
    try {
      if (useLocalFallback) {
        ...
        const nextItems = localItems.map(item =>
          item.id === reminder.id
            ? { ...item, isDone: !reminder.isDone, updatedAt: new Date().toISOString() }
            : item
        );
        writeLocalReminders(nextItems);
  ...
  const handleDelete = (id: string) => {
    showConfirm('Excluir Lembrete', 'Deseja remover este lembrete?', async () => {
      try {
        if (useLocalFallback) {
          const localItems = readLocalReminders().filter(item => item.id !== id);
  ...
  return (
    <div className={`rounded-2xl shadow-sm border border-gray-100 p-5 transition-all duration-200 ${cardBackgroundClass}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-gray-800">Lembretes</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-9 h-9 rounded-full border border-gray-200 text-gray-600 hover:text-gray-900 hover:border-gray-300 transition-colors flex items-center justify-center"
          ...
          <button
            onClick={openNew}
            className="w-9 h-9 rounded-full bg-brand-purple text-white hover:bg-purple-800 transition-colors flex items-center justify-center"
      ...
      {isCollapsed ? (
        <div
          className={`mt-4 rounded-lg border border-white/40 px-3 py-2 text-xs text-gray-600 transition-all duration-200 ${
            isExpandHovered ? 'bg-white backdrop-blur-none' : 'bg-white/30 backdrop-blur-sm'
          }`}
        >
          Lembretes ocultos.
        </div>
        ...
        {loading ? (
          ...
          <>
            {useLocalFallback && (
              <div className="text-xs text-gray-500 italic">Lembretes em modo local.</div>
            )}
            {reminders.length === 0 ? (
              <div className="text-sm text-gray-500 italic">Nenhum lembrete cadastrado.</div>
            ) : (
              reminders.map(reminder => (
                <div
                  key={reminder.id}
                  className={`flex items-start gap-3 p-3 rounded-lg border ${
                    reminder.isDone ? 'bg-gray-50 border-gray-200' : 'bg-amber-50 border-amber-100'
                  }`}
                >
                  <button
                    ...
                    title={reminder.isDone ? 'Marcar como pendente' : 'Marcar como concluido'}
                  >
                    {reminder.isDone ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${reminder.isDone ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                      {reminder.title}
                    </p>
